...
## 3. Ryzen Threadripper 3970X の 1コアあたり 1サイクルでの最大演算回数
...
- これにより、同じ命令を使う場合の演算回数が２倍になります

### 2. SIMD 命令

- SSE4、AVX2、AVX‑512 などの SIMD 命令を用いると、1命令で複数のデータ（レーン）を同時に演算できます。  
- **レーン数** は、1命令あたりに処理できる要素数を示します。

### 3. 命令発行の並列度（パイプライン数）

- 最近の CPUは、同一コア内に複数の FMA/SIMD パイプラインを備えています。  
- これにより、1サイクル内に複数の FMA 命令を同時発行（スループット向上）できます。
...
2. **FMA を使った 1命令あたりの FLOP**  
   - 倍精度：4 要素 × 2 演算（乗算＋加算）＝ **8 FLOP/命令**  
   - 単精度：8 要素 × 2 演算（乗算＋加算）＝ **16 FLOP/命令**

3. **サイクルあたりの FLOP（2本のパイプラインを考慮）**  
...
**補足・注意点**  
 ...
 - 実際の命令セットサポート状況やパイプライン構成の詳細は、AMD 社公式ドキュメントみるべきですが、正しい情報を得るのはかなり面倒です。また非常に専門的になっていて、必要な情報がわかりにくい場合もあります。wikichips.orgのような解説サイトを見るとわかりやすいかもしれません。
  - 例：[*A Look At The AMD Zen 2 Core*](https://fuse.wikichip.org/news/2458/a-look-at-the-amd-zen-2-core/2/)  
- たとえばChatGPTでWebSearchをonにして下調べしてから[Ryzen3970X理論性能](https://chatgpt.com/share/680467fd-acd8-800e-a567-e7e7f13b9f74) Google検索でクロスチェックするのも良いかもしれません。
