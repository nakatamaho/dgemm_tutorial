...
- **アーキテクチャ名**: Zen2
- **CPUパッケージの構成**：CPUパッケージは階層構造をとっています。まず、CPUコアが最小単位ですが、その一つ上の階層にCCX(Core Complex; 4個のコアと共有するL3)があります。それを二つまとめたものが Core Chiplet Die (or Core Complex Die; CCD)です。CCDたちはI/Oダイでつながっています。このような階層構造を取ることでCPU製造の歩留まりを上げることができます。つまりコア数も安定して増やすことが可能となっています（EPYCなどの製品群を見よ）。下の図を見つつ、Ryzen3970XはCCDあたりCCXが2個あり、CCXあたり4コア、つまりCCDあたり8コアあります。そしてCCDが4個あるので32コアとなっています。
...
- **L1/L2はコアごとにキャッシュを保持**:  コア当たりL1 32 KB データキャッシュ、L1 32 KB 命令キャッシュ、L2 512 KB
- **L3キャッシュはCCX単位で保持**: L3キャッシュはCCX単位で管理しています。つまり4コアで16 MB L3を共有します。全体としては16*2*4=128MBのL3キャッシュがあります。この16MBに後述する行列パネルを収めるのも一つの手です。
...
- **FMA パイプライン数**：2本（実行ポート 0 と 1）したがってSIMDを使わなくとも4FLOP/CYCLEが可能です。
