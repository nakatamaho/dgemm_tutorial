...
この節では、C++プログラムでBLAS dgemmと自作実装を比較検証する際の「デバッグ最小構成」を紹介します。DGEMMの高速な実装ではブロック化というテクニックが基本です。
しかしながら、ブロック化のデバッグは意外にも非常に困難です。特にブロックの範囲が行列のサイズを超えていたらすぐsegmentation faultで落ちます。この原因を探るのは大変厄介です。
なるべくデバッグしやすくするためのノウハウを紹介します。

## デバッグの基本ポイント

1. **Octave/Matlab形式で出力**: 計算結果を確認するため、行列をOctave/Matlab互換フォーマットで標準出力に表示.
   - 出力結果をコピペまたはパイプで流すことでoctaveの結果と比較できる。
2. **小さな整数値の使用**: -9〜9までの整数乱数を用いて、ターミナルで目視確認しやすいデータに限定。
   - 小さな整数値を使うことで、正確かつ、画面表示が見やすくする
   - 演算結果の行列は整数行列となるため、正解と誤差ゼロで比較できる
3. **番兵値の活用**: 行列外アクセスを検知するため、未使用領域を番兵値（-1000）で埋める
   - 大きな値が出た時にバグだとすぐわかる。
4. **小さな行列サイズ**: 行列サイズを意図的に4×4に固定し、手でも検証できるようにする。
5. **leading dimensionに値を加える**: 意図的にleading-dimensionを大きくし、ブロック化で出やすいleading dimension周りのバグをわかりやすくする。
   - バグがある場合、番兵値の影響で計算結果は大きくなりやすい。
