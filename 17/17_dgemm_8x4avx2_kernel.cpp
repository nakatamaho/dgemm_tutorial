...
// 6x6 micro kernel (using AVX2) - B transposed version
void avx2_micro_kernel_8x4_aligned(int k,
    ...
    for (int l = 0; l < k; ++l) {
        // Load two columns of A (8 elements total, 4 per vector)
        __m256d a0 = _mm256_load_pd(&A[l * lda]);
...
// DGEMM implementation using 8x4 micro kernel with transposed B panel, using L3 cache blocking
			  ...
			  double beta, double * __restrict C, int ldc)
{
    ...
    if (m % MR != 0 || n % NR != 0 || k % 8 != 0) {
        std::cerr << "Error: matrix dims must be multiples of 8." << std::endl;
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream csv_file("dgemm_benchmark_8x4avx2_kernel_results.csv");
    ...
    csv_file.close();
    std::cout << "Benchmark complete. Results saved to dgemm_benchmark_8x4avx2_kernel_results.csv" << std::endl;
