...
// 8x6 micro kernel (using AVX2) - B transposed version
void avx2_micro_kernel_8x6_aligned(int k,
    ...
    // First 4 rows of each column
    __m256d c0_0 = _mm256_loadu_pd(&C[0 + 0*ldc]);
    __m256d c1_0 = _mm256_loadu_pd(&C[0 + 1*ldc]);
    __m256d c2_0 = _mm256_loadu_pd(&C[0 + 2*ldc]);
    __m256d c3_0 = _mm256_loadu_pd(&C[0 + 3*ldc]);
    __m256d c4_0 = _mm256_loadu_pd(&C[0 + 4*ldc]);
    __m256d c5_0 = _mm256_loadu_pd(&C[0 + 5*ldc]);
    
    // Last 4 rows of each column
    __m256d c0_1 = _mm256_loadu_pd(&C[4 + 0*ldc]);
    __m256d c1_1 = _mm256_loadu_pd(&C[4 + 1*ldc]);
    __m256d c2_1 = _mm256_loadu_pd(&C[4 + 2*ldc]);
    __m256d c3_1 = _mm256_loadu_pd(&C[4 + 3*ldc]);
    __m256d c4_1 = _mm256_loadu_pd(&C[4 + 4*ldc]);
    __m256d c5_1 = _mm256_loadu_pd(&C[4 + 5*ldc]);
    
    // 2) Main loop over the shared dimension k
    for (int l = 0; l < k; ++l) {
        // Load two chunks of 4 elements from column l of A (8 elements total)
        __m256d a0 = _mm256_load_pd(&A[l * lda + 0]);
        __m256d a1 = _mm256_load_pd(&A[l * lda + 4]);
        
        // Broadcast each of the 6 B-elements from row l
        ...
        c1_0 = _mm256_fmadd_pd(a0, b1, c1_0);
        c2_0 = _mm256_fmadd_pd(a0, b2, c2_0);
        c3_0 = _mm256_fmadd_pd(a0, b3, c3_0);
        c4_0 = _mm256_fmadd_pd(a0, b4, c4_0);
        c5_0 = _mm256_fmadd_pd(a0, b5, c5_0);
        
        // FMA accumulation for last 4 rows
        c0_1 = _mm256_fmadd_pd(a1, b0, c0_1);
        c1_1 = _mm256_fmadd_pd(a1, b1, c1_1);
        c2_1 = _mm256_fmadd_pd(a1, b2, c2_1);
        c3_1 = _mm256_fmadd_pd(a1, b3, c3_1);
        c4_1 = _mm256_fmadd_pd(a1, b4, c4_1);
        c5_1 = _mm256_fmadd_pd(a1, b5, c5_1);
    }
    
    // 3) Store the updated accumulators back to C
    _mm256_storeu_pd(&C[0 + 0*ldc], c0_0);
    _mm256_storeu_pd(&C[4 + 0*ldc], c0_1);
    _mm256_storeu_pd(&C[0 + 1*ldc], c1_0);
    _mm256_storeu_pd(&C[4 + 1*ldc], c1_1);
    _mm256_storeu_pd(&C[0 + 2*ldc], c2_0);
    _mm256_storeu_pd(&C[4 + 2*ldc], c2_1);
    _mm256_storeu_pd(&C[0 + 3*ldc], c3_0);
    _mm256_storeu_pd(&C[4 + 3*ldc], c3_1);
    _mm256_storeu_pd(&C[0 + 4*ldc], c4_0);
    _mm256_storeu_pd(&C[4 + 4*ldc], c4_1);
    _mm256_storeu_pd(&C[0 + 5*ldc], c5_0);
    _mm256_storeu_pd(&C[4 + 5*ldc], c5_1);
}


// DGEMM implementation using 8x6 micro kernel with transposed B panel, using L3 cache blocking
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream csv_file("dgemm_benchmark_8x6avx2_kernel_results.csv");
    ...
    csv_file.close();
    std::cout << "Benchmark complete. Results saved to dgemm_benchmark_8x6avx2_kernel_results.csv" << std::endl;
