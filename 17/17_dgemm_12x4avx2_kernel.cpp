...
// 12x4 micro kernel (using AVX2) - B transposed version
void avx2_micro_kernel_12x4_aligned(int k,
    ...
    // Middle 4 rows (4-7)
    __m256d c4 = _mm256_loadu_pd(&C[4 + 0*ldc]);
    __m256d c5 = _mm256_loadu_pd(&C[4 + 1*ldc]);
    __m256d c6 = _mm256_loadu_pd(&C[4 + 2*ldc]);
    __m256d c7 = _mm256_loadu_pd(&C[4 + 3*ldc]);
    
    // Last 4 rows (8-11)
    __m256d c8 = _mm256_loadu_pd(&C[8 + 0*ldc]);
    __m256d c9 = _mm256_loadu_pd(&C[8 + 1*ldc]);
    __m256d c10 = _mm256_loadu_pd(&C[8 + 2*ldc]);
    __m256d c11 = _mm256_loadu_pd(&C[8 + 3*ldc]);
    
    // 2) Main loop over the shared dimension k
    for (int l = 0; l < k; ++l) {
        // Load three groups of 4 elements from column l of A
        __m256d a0 = _mm256_load_pd(&A[l * lda]);      // First 4 elements
        __m256d a1 = _mm256_load_pd(&A[l * lda + 4]);  // Next 4 elements
        __m256d a2 = _mm256_load_pd(&A[l * lda + 8]);  // Last 4 elements
        ...
        // FMA accumulation for middle 4 rows
        ...
        c8 = _mm256_fmadd_pd(a2, b0, c8);
        c9 = _mm256_fmadd_pd(a2, b1, c9);
        c10 = _mm256_fmadd_pd(a2, b2, c10);
        c11 = _mm256_fmadd_pd(a2, b3, c11);
    }
    ...
    // Middle 4 rows
    _mm256_storeu_pd(&C[4 + 0*ldc], c4);
    _mm256_storeu_pd(&C[4 + 1*ldc], c5);
    _mm256_storeu_pd(&C[4 + 2*ldc], c6);
    _mm256_storeu_pd(&C[4 + 3*ldc], c7);
    
    // Last 4 rows
    _mm256_storeu_pd(&C[8 + 0*ldc], c8);
    _mm256_storeu_pd(&C[8 + 1*ldc], c9);
    _mm256_storeu_pd(&C[8 + 2*ldc], c10);
    _mm256_storeu_pd(&C[8 + 3*ldc], c11);
}



// DGEMM implementation using 12x4 micro kernel with transposed B panel, using L3 cache blocking
            ...
            for (int i = 0; i < m; i += MC) {
                ...
                for (int jr = 0; jr < nc; jr += NR) {
                    ...
                    for (int ir = 0; ir < mc; ir += MR) {
                        ...
                        avx2_micro_kernel_12x4_aligned(
                            kc,
                            &Apanel[ir * kc], MR,
                            &Bpanel[jr * kc], NR,
                            Cblk, ldc);
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream csv_file("dgemm_benchmark_12x4avx2_kernel_results.csv");
    ...
    // Sizes that are multiples of 24
    ...
    csv_file.close();
    std::cout << "Benchmark complete. Results saved to dgemm_benchmark_12x4avx2_kernel_results.csv" << std::endl;
