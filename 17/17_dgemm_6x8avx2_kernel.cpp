...
// 6x8 micro kernel (using AVX2) - B transposed version
void avx2_micro_kernel_6x8_aligned(int k,
    ...
    // First 4 rows of C (rows 0-3)
    __m256d c00 = _mm256_loadu_pd(&C[0 + 0*ldc]);
    __m256d c01 = _mm256_loadu_pd(&C[0 + 1*ldc]);
    __m256d c02 = _mm256_loadu_pd(&C[0 + 2*ldc]);
    __m256d c03 = _mm256_loadu_pd(&C[0 + 3*ldc]);
    __m256d c04 = _mm256_loadu_pd(&C[0 + 4*ldc]);
    __m256d c05 = _mm256_loadu_pd(&C[0 + 5*ldc]);
    __m256d c06 = _mm256_loadu_pd(&C[0 + 6*ldc]);
    __m256d c07 = _mm256_loadu_pd(&C[0 + 7*ldc]);
    
    // Last 2 rows of C (rows 4-5)
    __m128d c10 = _mm_loadu_pd(&C[4 + 0*ldc]);
    __m128d c11 = _mm_loadu_pd(&C[4 + 1*ldc]);
    __m128d c12 = _mm_loadu_pd(&C[4 + 2*ldc]);
    __m128d c13 = _mm_loadu_pd(&C[4 + 3*ldc]);
    __m128d c14 = _mm_loadu_pd(&C[4 + 4*ldc]);
    __m128d c15 = _mm_loadu_pd(&C[4 + 5*ldc]);
    __m128d c16 = _mm_loadu_pd(&C[4 + 6*ldc]);
    __m128d c17 = _mm_loadu_pd(&C[4 + 7*ldc]);
    
    // 2) Main loop over the shared dimension k
    for (int l = 0; l < k; ++l) {
        // Load one column of A (first 4 elements) - use unaligned load
        __m256d a0 = _mm256_loadu_pd(&A[l * lda]);
        // Load one column of A (last 2 elements) - use unaligned load
        __m128d a1 = _mm_loadu_pd(&A[l * lda + 4]);
        
        // Broadcast each of the 8 B-elements from row l
        ...
        // FMA accumulation for first 4 rows
        ...
        c06 = _mm256_fmadd_pd(a0, b6, c06);
        c07 = _mm256_fmadd_pd(a0, b7, c07);
        
        // FMA accumulation for last 2 rows (using SSE)
        c10 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 0]), c10);
        c11 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 1]), c11);
        c12 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 2]), c12);
        c13 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 3]), c13);
        c14 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 4]), c14);
        c15 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 5]), c15);
        c16 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 6]), c16);
        c17 = _mm_fmadd_pd(a1, _mm_set1_pd(B[l * ldb + 7]), c17);
    }
    
    // 3) Store the updated accumulators back to C
    // Store first 4 rows
    _mm256_storeu_pd(&C[0 + 0*ldc], c00);
    _mm256_storeu_pd(&C[0 + 1*ldc], c01);
    _mm256_storeu_pd(&C[0 + 2*ldc], c02);
    _mm256_storeu_pd(&C[0 + 3*ldc], c03);
    _mm256_storeu_pd(&C[0 + 4*ldc], c04);
    _mm256_storeu_pd(&C[0 + 5*ldc], c05);
    _mm256_storeu_pd(&C[0 + 6*ldc], c06);
    _mm256_storeu_pd(&C[0 + 7*ldc], c07);
    
    // Store last 2 rows
    _mm_storeu_pd(&C[4 + 0*ldc], c10);
    _mm_storeu_pd(&C[4 + 1*ldc], c11);
    _mm_storeu_pd(&C[4 + 2*ldc], c12);
    _mm_storeu_pd(&C[4 + 3*ldc], c13);
    _mm_storeu_pd(&C[4 + 4*ldc], c14);
    _mm_storeu_pd(&C[4 + 5*ldc], c15);
    _mm_storeu_pd(&C[4 + 6*ldc], c16);
    _mm_storeu_pd(&C[4 + 7*ldc], c17);
}



// DGEMM implementation using 6x8 micro kernel with transposed B panel, using L3 cache blocking
			  ...
			  double beta, double * __restrict C, int ldc)
{
    if (m == 0 || n == 0 || ((alpha == 0.0 || k == 0) && beta == 1.0))
        return;
    if (m % MR != 0 || n % NR != 0 || k % 24 != 0) {
        std::cerr << "Error: matrix dims must be multiples of 24." << std::endl;
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream csv_file("dgemm_benchmark_6x8avx2_kernel_results.csv");
    ...
    // Sizes that are multiples of 24 (24 to 3500)
    for (int size = 24; size <= 3500; size += 24) {
    ...
    csv_file.close();
    std::cout << "Benchmark complete. Results saved to dgemm_benchmark_6x8avx2_kernel_results.csv" << std::endl;
