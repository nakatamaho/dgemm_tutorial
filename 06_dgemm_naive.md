...
1. **理論値に遠く及ばない性能**
  　- binary64(倍精度)で[1.89 TFLOPS](https://github.com/nakatamaho/dgemm_tutorial/blob/main/02_flops.md)が理論性能値でした。   
   - 実測の最大でも約 1.61 GFLOPS にとどまり，理論値の約 1.14 % にすぎません。  
   - なお，1コアかつ SIMD（AVX2） を使わない場合の理論ピークは約 7.4 GFLOPS となります。

2. **L1 キャッシュ内領域** ($$N \le 43$$)  
   - **平均性能**：4.55 GFLOPS （標準偏差 2.02）  
...
3. **L2 キャッシュ内領域** ($$44 \le N \le 128$$)  
   - **平均性能**：17.15 GFLOPS （標準偏差 2.13）  
   - **ピーク性能**：21.61 GFLOPS （$$N=52$$）  
   - L2 キャッシュ（256 KB）内にワーキングセットが収まることで，高い性能を達成しています。ばらつきは主にアライメントやキャッシュバンク競合によるものです。

4. **L3 キャッシュ内領域** ($$129 \le N \le 319$$)  
   - **平均性能**：16.53 GFLOPS （標準偏差 1.46）  
   - L3 キャッシュ（8 MB）内で動作しますが，L2 領域から約 3 GFLOPS 程度の性能低下が見られます。これはメモリアクセスレイテンシの影響が増大するためです。

5. **メインメモリ領域** ($$N \ge 320$$)  
   - **平均性能**：15.49 GFLOPS （標準偏差 0.66）  
   - ワーキングセットが L3 キャッシュを超え，主にメインメモリ帯域幅がボトルネックとなります。大規模行列では一部（$$N\approx600$$）で約 15.0 GFLOPS までわずかに低下しますが，全体としては比較的安定しています。
