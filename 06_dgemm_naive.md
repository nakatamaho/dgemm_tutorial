...
新たに取得したプロットおよびベンチマーク結果から、行列サイズ $$N$$ に対する DGEMM 性能はキャッシュ階層ごとに以下の４つの領域に分かれることが確認できます。各領域の境界は、ワーキングセットサイズ $$2N^2\times8$$ バイトとキャッシュ容量との関係からも概ね整合しています。
...
1. **理論値に遠く及ばない性能**
  ...
  - 1コアでSIMD(AVX2)を使わない場合は理論性能は7.4GFLOPSとなります。
...
3. **L2 キャッシュ内領域 ($$44 \le N \le 128$$)**  
   ...
   - L2 キャッシュ（256 KB）内にワーキングセットが収まることで、SIMDを使わない割には高い性能を達成しています。ばらつきは主にアライメントやキャッシュバンク競合によるものです。

4. **L3 キャッシュ内領域 ($$129 \le N \le 319$$)**  
   - **平均性能**：16.53 GFLOPS （標準偏差 1.46）  
   - L3 キャッシュ（8 MB）容量内で維持されますが、L2 領域から約 3 GFLOPS 程度の性能低下が見られます。これはメモリアクセスレイテンシの影響が増大するためです。

5. **メインメモリ領域 ($$N \ge 320$$)**  
   - **平均性能**：15.49 GFLOPS （標準偏差 0.66）  
   - ワーキングセットが L3 キャッシュ容量を超え、主にメインメモリ帯域幅がボトルネックとなります。大規模行列においては一部 $$N\approx600$$ 前後で約 15.0 GFLOPS までわずかに低下する箇所がありますが、全体としては比較的安定しています。
