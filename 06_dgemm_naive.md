...
1. **理論値に遠く及ばない性能**
  　- binary64(倍精度)で[1.89 TFLOPS](https://github.com/nakatamaho/dgemm_tutorial/blob/main/02_flops.md)が理論性能値でした。   
   - 実測の最大でも約 6.86 GFLOPS にとどまり，理論値の約 0.36 % にすぎません。  
   - なお，1コアかつ SIMD（AVX2） を使わない場合の理論ピークは約 7.4 GFLOPS となります。

2. **L1 キャッシュ内領域** ($$8 \le N \le 43$$)
   - L1 キャッシュ容量：32 KB （$$2N^2\times8 \le 32\,768$$ より $$N\lesssim\sqrt{32\,768/16}\approx45$$）
...
3. **L2 キャッシュ内領域** ($$44 \le N \le 128$$)  
   - L2 キャッシュ容量：256 KB （$$2N^2\times8 \le 262\,144$$ より $$N\lesssim128$$）  
   - **平均性能**：6.154 GFLOPS（標準偏差 0.538）  
   - **最小／最大**：4.793 GFLOPS ／ 6.858 GFLOPS（$$N=59$$）

4. **L3 キャッシュ内領域** ($$129 \le N \le 319$$)  
   - L3 キャッシュ容量：8 MB 理論境界は $$N\lesssim724$$ だが，実測では約 $$N=319$$ 付近で性能低下が始まる  
   - **平均性能**：6.275 GFLOPS（標準偏差 0.145）  
   - **最小／最大**：5.913 GFLOPS ／ 6.446 GFLOPS（$$N=310$$）

5. **メインメモリ領域** ($$N \ge 320$$)  
   - ワーキングセットが L3 を超えて主記憶へのアクセスが支配的  
   - **平均性能**：6.692 GFLOPS（標準偏差 0.095）  
   - **最小／最大**：6.476 GFLOPS（$$N=324$$）／6.796 GFLOPS（$$N=597$$）  
