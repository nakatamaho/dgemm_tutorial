...
プロットおよびベンチマーク結果から，行列サイズ $$N$$ に対する DGEMM 性能はキャッシュ階層ごとに以下の５つの領域に分類でき，それぞれの境界はワーキングセットサイズ  
$$2N^2 \times 8\ \text{bytes}$$  
とキャッシュ容量との関係ともよく整合しています。

1. **理論値に遠く及ばない性能**
   - binary64(倍精度)で[1.89 TFLOPS](https://github.com/nakatamaho/dgemm_tutorial/blob/main/02_flops.md)が理論性能値でした。
   - 1コアでFMAを使い、SIMD（AVX2）を使わない場合の理論ピークは 7.4 GFLOPS となります(1.8944  * 1000 / 32 (cores) / 16 (AVX2;SIMD) * 2 (FMA) = 7.4)。 
   - 実測の最大は約 6.86 GFLOPS にとどまり，理論値の0.36 % にすぎません。
   - ただ1コア、SIMDなしFMAありの場合の理論性能値では、92.7%も出ていました(Turboも入っているため正確ではない)。

3. **L1 キャッシュ内領域** ($$8 \le N \le 43$$)
   - L1 キャッシュ容量：32 KB （
$$2N^2\times8 \le 32\,768$$ より
$$N\lesssim\sqrt{32\,768/16}\approx45$$）
   - **平均性能**：3.551 GFLOPS（標準偏差 1.674）
   - 行列サイズの増加に伴い性能は概ね上昇しますが，ループオーバーヘッドやキャッシュ競合の影響でばらつきが大きいです。  
...
$$44 \le N \le 128$$)  
   - L2 キャッシュ容量：256 KB （
$$2N^2\times8 \le 262\,144$$ より
$$N\lesssim128$$）  
   - **平均性能**：6.154 GFLOPS（標準偏差 0.538）  
   - **最小／最大**：4.793 GFLOPS ／ 6.858 GFLOPS（
$$N=59$$）

5. **L3 キャッシュ内領域** ($$129 \le N \le 319$$)  
   - L3 キャッシュ容量：8 MB 理論境界は
$$N\lesssim724$$ だが，実測では約
$$N=319$$ 付近で性能低下が始まる  
   - **平均性能**：6.275 GFLOPS（標準偏差 0.145）  
   - **最小／最大**：5.913 GFLOPS ／ 6.446 GFLOPS（
$$N=310$$）

6. **メインメモリ領域** (
$$N \ge 320$$)  
   - ワーキングセットが L3 を超えて主記憶へのアクセスが支配的  
   - **平均性能**：6.692 GFLOPS（標準偏差 0.095）  
   - **最小／最大**：6.476 GFLOPS（
$$N=324$$）／6.796 GFLOPS（
$$N=597$$）  
   - 備考：プリフェッチ効果などにより，L3 超過後も若干性能が向上する傾向があります。

## 注意点
1. gcc の-O3だと、FMA,AVX2の命令を吐きました。すると最大21.8864 GFLOPS (n=52)、平均は14.5531 GFLOPSでした。FMA,AVX2使用時の1coreの理論性能値は59.2GFLOPSなので、25%程度の性能となりました。
