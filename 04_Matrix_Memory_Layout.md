...
C/C++でどうやって行列を表現するかというのには、様々な方法がありえます。互換性、メモリの連続性、汎用性などを考えると、Column-Majorで一次元配列に開くというのがベストです。
...
C/C++で2次元を配列を実現する最も良いアプローチは、2次元配列を1次元配列として表現し、明示的にインデックス計算を行うことです。以下のようなコードになります。$`m \times n`$行列の$`C_{ij}`$にアクセスするには、$`i`$, $`j`$だけでなく、leading dimensionである$`m`$をインデックス計算のため明示的に使います。
