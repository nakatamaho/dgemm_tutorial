...
3. **L2キャッシュをフルに生かす**: L2キャッシュからのデータ供給速度が演算より高速になるように、つまり、いつでも演算器が回っていても大丈夫なようにしています。今回はAVX2は使わないのでデータ供給スピードは行列がL2キャッシュに入っている間は保たれるようになっています。
4. **L3キャッシュは生かせない実装**: A, Bの幅$`k`$が大きくなると、行列A, BはL2のキャッシュの外に出ます。すると、このrank1 アップデートの目論見ははずれ、パフォーマンスが落ちるはずです。
