...
演算器が全力(16 FLOP/cycle)で走っているとき、L1データキャッシュのロードポートも全力(8 double/cycle読み、4 double/cycle書き)で走ればちょうど計算が回るので、L1 は「小さいけれど必要十分に高速なキャッシュ」として十分役目を果たすからです。
もう少し詳しく言うと 
...
- 逆に考えると、Ryzen3970Xのようにbinary64で1クロック16FLOP計算可能とします。何かの計算をする、というのは二つの数のFMAです。つまり乗算と加算という二回の演算を8個のbinary64について行えば8x2=16FLOP演算となり、演算器を埋めることができます。そして結果の書き込みも4個のbinary64をL1キャッシュに可能です。最低、L1からCPUに1クロックでbinary64を8個読み込め、binary64を4個書き込みできないようなアーキテクチャでは、そもそも演算器が無駄になります。従ってそのようなCPUの設計は絶対にしないのです。
