...
余談ですが、最近筆者は大量のデータ処理をしています。ディスクから大量のデータを読み出し計算が終わっているいないを確認した後、終わっていない場合、非常に小さい計算のインプットファイルたちを中主出するというものです。これにはHDDを使うと非常に低速です。高速なSSDを使うと処理が数倍になります。ですが、ごく少量のデータ処理であれば、RAM に作業用ディスクを置けばいい。そうすると上のスケール表をみると、どんなSSDを使ってもRAMディスクに置いた方が高速です。このような「感覚」を持っておくのはコンピュータを有効活用する場合にとても重要になります。
