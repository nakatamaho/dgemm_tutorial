...
1. **CPU レジスタ**: 1秒（基準）
   - 実際のアクセス時間: 約0.27 ns（1サイクル）

2. **L1 キャッシュ**: 約4〜8秒
   - 実際のアクセス時間: 整数操作 1.08〜1.35 ns（4〜5サイクル）
   - 実際のアクセス時間: 浮動小数点操作 1.89〜2.16 ns（7〜8サイクル）

3. **L2 キャッシュ**: 約12秒以上
   - 実際のアクセス時間: 3.24 ns以上（12サイクル以上）

4. **L3 キャッシュ**: 約39秒
   - 実際のアクセス時間: 約10.54 ns（平均39サイクル）

5. **メインメモリ (RAM)**: 約37〜226秒
   - 実際のアクセス時間: ヒット時 約10〜15 ns（オープンページ）
   - 実際のアクセス時間: ミス時 約38〜61 ns（別ページ）

6. **ディスク**:
   - **SSD**: 約10時間〜4日
     - 実際のアクセス時間: 約0.01〜0.1 ms
   - **HDD**: 約4日〜43日
     - 実際のアクセス時間: 約0.1〜10 ms
