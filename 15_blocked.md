...
行列乗算（DGEMM）のような大規模な計算では、メモリアクセスがパフォーマンスの主要なボトルネックとなります。ブロック化とは、大きな行列を小さなブロックに分割して処理することで、キャッシュの局所性を高める手法です。三段階のブロック化をします。おおまかにL3キャッシュのブロック化、L2/L3パネルのブロック化、L1とレジスタのブロック化となります。
...
2. **パネルパッキング**:連続的なメモリアクセスのためのデータ再配置
3. **レジスタブロック化**:MR, NRサイズのマイクロカーネル
...
| **M→N→K** | `C` ブロックが L2/L3 に残らず毎回 DRAM へ往復が必要 Cは各スレッドのキャッシュに収める必要|
